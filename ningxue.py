...
def waktu(secs):
    ...
    return '%02d 小時 %02d 分 %02d 秒' % (hours, mins, secs)      
...
#                            Name = summon(op.param2)
		...
		if msg.from_ in admin:
		    ...
		    ningxue.sendText(msg.to,"只限管理員")
                ...
                md = ""
		if wait["Sambutan"] == True: md+="╠➩✔️ 進群歡迎 : On\n"
		else:md+="╠➩❌ 進群歡迎 : Off\n"
		...
		if wait["Contact"] == True: md+="╠➩✔️ Info Contact : On\n"
                ...
                if wait["inviteprotect"] == True:md+="╠➩✔️ 邀請保護 : On\n"
                else: md+= "╠➩❌ 邀請保護 : Off\n"                
		if wait["Qr"] == True: md+="╠➩✔️ Qr保護 : On\n"
		else:md+="╠➩❌ Qr保護 : Off\n"
		if wait["AutoKick"] == True: md+="╠➩✔️ 自動踢出 : On\n"
		else:md+="╠➩❌ 自動踢出 : Off\n"
		if wait["alwaysRead"] == True: md+="╠➩✔️ Always Read : On\n"
		else:md+="╠➩❌ Always Read: Off\n"
		if wait["detectMention"] == True: md+="╠➩✔️ 標註訊息回復1 : On\n"
		else:md+="╠➩❌ 標註訊息回復1 : Off\n"		
		if wait["detectMention2"] == True: md+="╠➩✔️ 標註訊息回復2 : On\n"
		else:md+="╠➩❌ 標註訊息回復2 : Off\n"	
		if wait["detectMention3"] == True: md+="╠➩✔️ 標註訊息回復3 : On\n"
		else:md+="╠➩❌ 標註訊息回復3 : Off\n"			
		...
		if msg.from_ in admin:
                    ...
                    if msg.toType == 2:
                      if msg.from_ in admin:
                        ...
                        for tag in wait["blacklist"]:
                            matched_list+=filter(lambda str: str == tag, gMembMids)
                        if matched_list == []:
                            ningxue.sendText(msg.to,"幹你娘 沒有人我踢3小")
                            return
                        for jj in matched_list:
                            try:
                                ningxue.kickoutFromGroup(msg.to,[jj])
		    ...
		    if msg.from_ in Creator:
                     if msg.toType == 2:
                        ...
                        gs = ningxue.getGroup(msg.to)
                        ningxue.sendText(msg.to,"毅毅幻滅開始~~~~")
                        targets = []
                        for g in gs.members:
